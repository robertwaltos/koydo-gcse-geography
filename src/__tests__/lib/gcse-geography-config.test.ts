...
import { EXAM_CONFIG } from "@/lib/gcse-geography/config";

describe("GCSE Geography Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM192");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("gcse-geography");
  });
